...
| Branch | Status |
|--------|--------|
| `origin/main` (HEAD=1283a2d) | 2260+ commits, has `ManagedStreamSet` + `ManagedOrderBookStream` inline in `binance.py` (1026 lines), plus `price_safety_net.py`, `price_freshness.py`, and `test_price_safety_net.py` |
...
### Tests
- **Full suite: 826 passed, 7 failed, 6 warnings** (51.06s)
...
2. **`ENVIRONMENT_STATUS.md`** is untracked (not committed yet)
