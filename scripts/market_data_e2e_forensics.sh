...
canonical=details.get('canonical_market_data') or last.get('canonical_market_data') or {}
deep=details.get('deep_market_data') or last.get('deep_market_data') or {}
persist=details.get('canonical_persistence') or last.get('canonical_persistence') or {}
transport=details.get('transport_telemetry') or last.get('transport_telemetry') or {}
...
add('canonical websocket state','PASS' if state=='connected' and h.get('connected') is True else ('FAIL' if state in ('degraded','reconnecting','connecting') else 'WARN'),state)
for m in ('received_events','accepted_events','published_events'):
    v=h.get(m); add(m,'PASS' if isinstance(v,int) and v>0 else 'WARN',v)
...
age=h.get('receive_to_now_age_ms'); add('latest receive freshness','PASS' if isinstance(age,(int,float)) and age<15000 else ('FAIL' if age is not None else 'WARN'),age)
summary={}
for key,vals in streams.items():
    ...
    add(f'{key} growth','PASS' if s['delta']>0 else 'WARN',s)
...
for key in ('stream:market.trade','stream:market.book.snapshot'):
    ...
    if raw:
        ...
        while i+1<len(toks):
            ...
            i+=1
        groups[key]=parsed
result={'checks':checks,'health':{'canonical_market_data':canonical,'deep_market_data':deep,'canonical_persistence':persist,'transport_telemetry':transport},'streams':summary,'consumer_groups':groups,'samples':len(samples)}
