...
for c in "$REDIS_CONTAINER" "$PAPER_CONTAINER"; do
  if ! docker inspect "$c" >/dev/null 2>&1; then fail "missing container: $c"; continue; fi
  status="$(docker inspect --format '{{.State.Status}}' "$c")"; health="$(docker inspect --format '{{if .State.Health}}{{.State.Health.Status}}{{else}}none{{end}}' "$c")"
  echo "$c status=$status health=$health"; [ "$status" = running ] || fail "$c is not running"; [ "$health" = healthy ] || warn "$c health is $health"
done
echo '--- Redis consumer-group pending entries ---'
if docker inspect "$REDIS_CONTAINER" >/dev/null 2>&1; then
  while IFS= read -r key; do
    [ -n "$key" ] || continue
    docker exec "$REDIS_CONTAINER" redis-cli --json XINFO GROUPS "$key" 2>/dev/null | MAX_PENDING="$MAX_PENDING" python3 -c 'import json,os,sys
for g in json.load(sys.stdin):
    pending=int(g.get("pending",0)); print(f"{g.get(\"name\",\"\")}: pending={pending} lag={g.get(\"lag\")}")
    if pending > int(os.environ["MAX_PENDING"]): raise SystemExit(2)' || { rc=$?; [ "$rc" -eq 2 ] && fail "consumer-group pending exceeds ${MAX_PENDING}: $key"; }
...
redis_stats="$(docker exec "$REDIS_CONTAINER" redis-cli INFO stats 2>/dev/null || true)"; redis_memory="$(docker exec "$REDIS_CONTAINER" redis-cli INFO memory 2>/dev/null || true)"
...
for c in "$REDIS_CONTAINER" "$PAPER_CONTAINER"; do
  ...
  if [ -n "$stats" ]; then
    periods="$(printf '%s\n' "$stats" | awk '$1=="nr_periods"{print $2}')"; throttled="$(printf '%s\n' "$stats" | awk '$1=="nr_throttled"{print $2}')"
  ...
  else warn "$c CPU cgroup telemetry unavailable"; fi
done
echo '--- Canonical application health ---'
if curl -fsS --max-time 5 http://127.0.0.1:8090/health >/tmp/aitos-health.json 2>/dev/null; then
  python3 - <<'PY'
import json
p='/tmp/aitos-health.json'; data=json.load(open(p))
modules=data.get('modules',[]) if isinstance(data,dict) else []
module=next((m for m in modules if m.get('module_id')=='data-ingestion-service'),None)
canonical=(module or {}).get('details',{}).get('canonical_market_data')
# Backward compatibility for older health payloads.
if not canonical:
    canonical=data.get('details',{}).get('canonical_market_data') or data.get('canonical_market_data')
if not canonical:
    raise SystemExit('missing canonical market-data health from data-ingestion-service /health')
...
if h.get('received_events',0) <= 0:
    raise SystemExit('no canonical WebSocket events received')
if h.get('published_events',0) <= 0:
    raise SystemExit('no canonical events published')
PY
else
