...
from aitos.security.secrets import EnvironmentSecretProvider, VaultSecretProvider, build_secret_provider


def test_environment_provider_fails_closed_for_required_secret(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_vault_provider_reads_kv_v2(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("aitos.security.secrets.urlopen", lambda *args, **kwargs: Response())
    provider = VaultSecretProvider("http://vault", "token")
    assert provider.get("API_KEY") == "secret"


def test_vault_backend_requires_token_file(monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
