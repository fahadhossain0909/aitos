...
def test_stale_update_is_ignored_without_mutating_sequence():
    ...
    book.seed(snap(100))
    book.apply(DepthUpdate(101, 102, 100, ((100.0, 7.0),), (), 1000))
    # Stale update (final_update_id <= last_update_id) returns None silently.
    result = book.apply(DepthUpdate(90, 100, 0, ((100.0, 1.0),), (), 900))
    assert result is None
    assert book.last_update_id == 102
    assert book.snapshot().bids[0] == (100.0, 7.0)
...
def test_valid_multi_update_chain_replays_in_order():
    ...
    book.seed(snap(200))
    updates = [
        DepthUpdate(201, 202, 200, ((100.0, 6.0),), (), 1000),
        DepthUpdate(203, 204, 202, ((99.0, 4.0),), (), 1100),
...
def test_pu_is_authoritative_even_when_first_update_id_jumps():
    book = LocalOrderBook("BTCUSDT")
    book.seed(snap(200))
    book.apply(DepthUpdate(201, 202, 200, (), (), 1000))
    ...
    result = book.apply(DepthUpdate(203, 205, 202, ((100.0, 6.0),), (), 1100))
...
def test_gap_requires_resync_before_new_updates_can_be_applied():
    ...
    book.seed(snap(306))
    result = book.apply(DepthUpdate(307, 308, 306, ((100.0, 8.0),), (), 1100))
    assert result.last_update_id == 308
...
def test_chain_break_after_reconnect_is_rejected():
    book = LocalOrderBook("BTCUSDT")
    book.seed(snap(400))
    book.apply(DepthUpdate(401, 402, 400, (), (), 1000))
    with pytest.raises(OrderBookSequenceError):
        book.apply(DepthUpdate(403, 404, 123, (), (), 1100))


def test_reseed_clears_old_levels():
    book = LocalOrderBook("BTCUSDT")
    book.seed(snap(500))
    book.apply(DepthUpdate(501, 502, 500, ((98.0, 9.0),), (), 1000))
    book.seed(snap(600))
    result = book.apply(DepthUpdate(601, 602, 600, (), (), 1100))
    assert result.last_update_id == 602
