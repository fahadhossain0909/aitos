...
from aitos.data.aitos_bridge import canonical_trade_to_domain, book_events_to_snapshot
...
def test_canonical_trade_maps_to_existing_trade_tick():
    event = CanonicalTrade(
        "binance", "futures_um", "BTCUSDT", "42",
        datetime(2026, 1, 1, tzinfo=timezone.utc), 100.0, 2.5, "sell", True,
    )
...
def test_book_events_map_to_sorted_snapshot_and_remove_zero_level():
    ...
    events = [
        ...
        CanonicalBookEvent("binance", "futures_um", "BTCUSDT", 1, ts, "sell", 101.0, 3.0),
