...
def test_replay_routes_trade_and_book_events():
    seen = []
    runner = AITOSReplay(
        on_trade=lambda e: seen.append(("trade", e.trade_id)),
        on_book=lambda e: seen.append(("book", e.update_id)),
    )
    events = [
        CanonicalTrade(
            "binance",
...
def test_stale_book_update_is_not_forwarded():
    ...
    runner.feed_book(
        CanonicalBookEvent("binance", "futures_um", "BTCUSDT", 10, ts, "buy", 99, 1)
    )
    assert not runner.feed_book(
        CanonicalBookEvent("binance", "futures_um", "BTCUSDT", 9, ts, "buy", 99, 2)
    )
    assert seen == [10]
