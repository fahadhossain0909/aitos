...
class TradeTick:
    ...
    def from_dict(cls, data: dict[str, Any]) -> TradeTick:
        ...
        if isinstance(raw_side, TradeSide):
            ...
            raise KeyError("side")
        return cls(symbol=data["symbol"], trade_id=int(data["trade_id"]), price=float(data["price"]), quantity=float(data["quantity"]), side=side, is_buyer_maker=bool(data.get("is_buyer_maker", side is TradeSide.SELL)), timestamp=_dt(data["timestamp"]))
