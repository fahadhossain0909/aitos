...
class CrossMarketIntelligenceEngine:
    ...
    def __init__(self, max_samples: int = 2048) -> None:
        ...
        self._series: dict[str, deque[float]] = defaultdict(lambda: deque(maxlen=max_samples))
    ...
    def discover_lead_lag(self, leader: str, follower: str, max_lag: int = 12) -> LeadLagResult:
        best_lag, best_corr = 0, 0.0
        for lag in range(max_lag + 1):
            corr = self.correlation(leader, follower, lag)
            if abs(corr) > abs(best_corr):
                best_lag, best_corr = lag, corr
        return LeadLagResult(leader, follower, best_lag, best_corr, min(len(self.returns(leader)), len(self.returns(follower))))

    def build_state(self, *, volatility: float, risk: float, liquidity: float, confidence: float = 0.5, features: dict[str, float] | None = None) -> GlobalMarketState:
        return MarketStateBuilder.build(volatility=volatility, risk=risk, liquidity=liquidity, confidence=confidence, features=features)
