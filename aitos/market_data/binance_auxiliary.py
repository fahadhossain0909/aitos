...
class BinanceAuxiliaryMarketDataAdapter(BinanceCanonicalMarketDataAdapter):
    ...
    async def stream_funding(self, symbols: list[str]) -> AsyncIterator[MarketEvent]:
        ...
        async for data, stream_name in self.exchange._raw_stream(streams, emit_reconnect=True):
            symbol = stream_name.split("@", 1)[0].upper()
            payload = {"symbol": symbol, "funding_rate": float(data.get("r", 0.0)), "mark_price": float(data.get("p", 0.0)), "index_price": float(data.get("i", 0.0)), "next_funding_time": data.get("T"), "event_time": data.get("E")}
    ...
    async def stream_liquidations(self, symbols: list[str] | None = None) -> AsyncIterator[MarketEvent]:
        ...
        async for data, _ in self.exchange._raw_stream(["!forceOrder@arr"], emit_reconnect=True):
            ...
            for item in orders:
                ...
                payload = {"symbol": symbol, "side": order.get("S"), "order_type": order.get("o"), "time_in_force": order.get("f"), "quantity": float(order.get("q", 0.0)), "price": float(order.get("p", 0.0)), "average_price": float(order.get("ap", 0.0)), "status": order.get("X"), "event_time": order.get("T")}
        ...
        while True:
            for symbol in bounded:
                try:
                    ...
                    yield _event(MarketEventType.OPEN_INTEREST, oi.symbol, {"symbol": oi.symbol, "open_interest": oi.open_interest, "timestamp": oi.timestamp.isoformat()}, oi.timestamp)
        ...
        while True:
            try:
                ...
                for symbol, value in filters.items():
                    payload = {"symbol": symbol, "tick_size": value.tick_size, "step_size": value.step_size, "min_notional": value.min_notional, "quantity_precision": value.quantity_precision, "price_precision": value.price_precision}
