...
class BinanceAuxiliaryMarketDataRuntime:
    ...
    def __init__(self, adapter: BinanceAuxiliaryMarketDataAdapter, gateway: MarketDataGateway, symbols: list[str]) -> None:
    ...
    async def start(self) -> None:
        ...
        self._drain_tasks = [asyncio.create_task(self._drain_loop(), name="market-data-auxiliary-drain")]
        ...
        self._start("open_interest", lambda: self.adapter.stream_open_interest(self.symbols))
        ...
        logger.info("Binance auxiliary market-data runtime started", extra={"aitos_extra": {"symbols": self.symbols}})

    def _start(self, name: str, factory: Callable) -> None:
        self._tasks.append(asyncio.create_task(self._run(name, factory), name=f"market-data-{name}"))
    ...
    async def _drain_loop(self) -> None:
        while not self._stopped:
            try:
                ...
                logger.warning("auxiliary gateway drain failed", extra={"aitos_extra": {"error": str(exc)}})
        ...
        while not self._stopped:
            ...
            try:
                ...
                while not self._stopped:
                    ...
                    if event.source == MarketSource.WEBSOCKET: self.gateway.mark_connected()
                ...
                if not self._stopped:
                    ...
                    logger.warning("auxiliary market-data producer failed", extra={"aitos_extra": {"stream": name, "error": str(exc), "delay": delay}})
            finally:
    ...
    async def update_symbols(self, symbols: list[str] | tuple[str, ...]) -> bool:
        ...
        async with self._lock:
            if normalized == self.symbols: return False
            self.symbols = normalized
            if self._stopped: return True
            names = {"ticker", "funding", "open_interest"}
            tasks = [t for t in self._tasks if t.get_name().removeprefix("market-data-") in names]
    ...
    async def stop(self) -> None:
        ...
        self._tasks.clear()
        for task in self._drain_tasks: task.cancel()
        if self._drain_tasks: await asyncio.gather(*self._drain_tasks, return_exceptions=True)
        self._drain_tasks.clear()
        self.gateway.stop()

    def snapshot(self) -> dict[str, object]:
        return {"running": not self._stopped, "symbols": list(self.symbols), "active_producers": [t.get_name() for t in self._tasks if not t.done()], "gateway": self.gateway.snapshot()}
