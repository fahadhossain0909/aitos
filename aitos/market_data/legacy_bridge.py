...
The bridge lets the existing ingestion transport feed the v1 market-data plane
without opening a second WebSocket connection. Once consumers are migrated,
the legacy EventBus topics can be removed safely.
"""
...
from .contracts import BookLevel, MarketEvent, MarketEventType, MarketSource
...
def book_snapshot_event(book: LegacyBook, *, market_type: str = "futures") -> MarketEvent:
    ...
    return MarketEvent(
        ...
        payload={
            "bids": [BookLevel(p, q).__dict__ for p, q in book.bids],
            "asks": [BookLevel(p, q).__dict__ for p, q in book.asks],
