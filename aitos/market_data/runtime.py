...
class CanonicalMarketDataRuntime:
    ...
    async def start(self) -> None:
        ...
        logger.info("canonical market-data runtime started", extra={"aitos_extra": {"venue": self.adapter.venue.value, "market_type": self.adapter.market_type.value, "trade_symbols": self.symbols, "orderbook_symbols": self.orderbook_symbols, "kline_symbols": self.kline_symbols, "kline_timeframe": KLINE_TIMEFRAME, "orderbook_levels": self.orderbook_levels, "orderbook_fallback_levels": self.orderbook_fallback_levels, "enable_trades": self.enable_trades, "enable_orderbooks": self.enable_orderbooks, "enable_klines": self.enable_klines, "stream_idle_timeout_seconds": self.stream_idle_timeout_seconds, "gateway_drain_workers": GATEWAY_DRAIN_WORKERS}})
    ...
    async def update_kline_symbols(self, symbols: list[str] | tuple[str, ...]) -> bool:
        ...
        async with self._reconfigure_lock:
            ...
            if self._stopped:
                return True
            kline_tasks = [task for task in self._tasks if task.get_name() == "market-data-klines"]
            ...
            self._start_kline_task()
            logger.info("live kline subscription reconfigured", extra={"aitos_extra": {"stage": "kline_subscription_reconfigured", "previous_symbols": previous, "kline_symbols": normalized, "kline_timeframe": KLINE_TIMEFRAME}})
            return True
    ...
    async def update_orderbook_symbols(self, symbols: list[str] | tuple[str, ...]) -> bool:
        ...
        async with self._reconfigure_lock:
            ...
            logger.info("live orderbook subscription reconfigured", extra={"aitos_extra": {"stage": "subscription_reconfigured", "previous_symbols": previous, "orderbook_symbols": normalized, "orderbook_levels": self.orderbook_levels}})
            return True
    ...
    async def update_orderbook_levels(self, levels: int, reason: str) -> bool:
        ...
        async with self._reconfigure_lock:
            ...
            if not self._stopped:
                await self._restart_orderbook_task()
            logger.warning("live orderbook depth changed", extra={"aitos_extra": {"stage": "orderbook_depth_reconfigured", "previous_levels": previous, "orderbook_levels": levels, "reason": reason}})
            return True

    async def _restart_orderbook_task(self) -> None:
        orderbook_tasks = [task for task in self._tasks if task.get_name() == "market-data-orderbook"]
        ...
        while not self._stopped:
            try:
                ...
                if published and not self._first_canonical_publish_seen:
                    ...
                    logger.info("canonical market-data first publish completed", extra={"aitos_extra": {"stage": "first_canonical_publish", "worker_id": worker_id}})
                await self._adapt_orderbook_depth()
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                logger.warning("canonical market-data publish failed; dropping failed event and continuing", extra={"aitos_extra": {"stage": "canonical_publish_error", "error": str(exc), "worker_id": worker_id}})
    ...
    async def _run(self, stream_name: str, stream_factory: Callable[[], AsyncIterator]) -> None:
        ...
        while not self._stopped:
            ...
            try:
                ...
                logger.info("canonical stream starting", extra={"aitos_extra": {"stage": "stream_start", "stream": stream_name, "reconnect_delay": delay, "consecutive_failures": state["consecutive_failures"]}})
                ...
                while not self._stopped:
                    try:
                        ...
                        logger.error("canonical market-data stream watchdog timeout; reconnecting", extra={"aitos_extra": {"stage": "canonical_idle_timeout", "stream": stream_name, "timeout_seconds": self.stream_idle_timeout_seconds, "consecutive_failures": state["consecutive_failures"]}})
                        raise exc
                    saw_event = True
                    ...
                    if accepted and event.source == MarketSource.WEBSOCKET:
                        if not self._first_canonical_event_seen:
                            ...
                            logger.info("canonical market-data first event accepted", extra={"aitos_extra": {"stage": "first_canonical_event", "stream": stream_name, "event_type": type(event).__name__, "source": event.source.value}})
                ...
                logger.warning("canonical stream ended unexpectedly", extra={"aitos_extra": {"stage": "canonical_stream_end", "stream": stream_name}})
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                if self._stopped:
                ...
                logger.exception("canonical market-data stream failed; reconnecting", extra={"aitos_extra": {"stage": "canonical_stream_error", "stream": stream_name, "error_type": type(exc).__name__, "error": str(exc), "delay": delay, "failure_kind": failure_kind}})
            finally:
            ...
            if failure_kind in {"idle_timeout", "stream_end"} or not saw_event:
                ...
                delay = min(max(delay * 2 if int(state["consecutive_failures"]) > 1 else delay, _RECONNECT_INITIAL_DELAY_SECONDS), _RECONNECT_MAX_DELAY_SECONDS)
            else:
                if elapsed >= RECONNECT_STABLE_SECONDS:
                    ...
                    state["consecutive_failures"] = int(state["consecutive_failures"]) + 1
                    delay = min(max(delay * 2, _RECONNECT_INITIAL_DELAY_SECONDS), _RECONNECT_MAX_DELAY_SECONDS)
            state["restarts"] = int(state["restarts"]) + 1
            logger.warning("canonical stream reconnect scheduled", extra={"aitos_extra": {"stage": "canonical_reconnect_scheduled", "stream": stream_name, "failure_kind": failure_kind, "delay": delay, "consecutive_failures": state["consecutive_failures"], "stream_uptime_seconds": round(elapsed, 3)}})
