...
class BinanceFuturesAdapter(ExchangeAdapter):
    ...
    async def stream_klines(self, symbols: list[str], timeframe: str) -> AsyncIterator[Kline]:
        async for item in self._stream([f"{s.lower()}@kline_{timeframe}" for s in symbols], parse_kline_ws):
            yield item

    async def stream_trades(self, symbols: list[str]) -> AsyncIterator[TradeTick]:
        if not symbols:
            return
        async for data, _ in self._raw_stream([f"{s.lower()}@aggTrade" for s in dict.fromkeys(symbols)], emit_reconnect=True):
            try:
                yield parse_agg_trade_ws(data)
            except Exception as exc:
                logger.error("Binance aggregate-trade event invalid", extra={"aitos_extra": {"error": str(exc)}})
    ...
    async def _direct_raw_stream(self, stream: str, emit_reconnect: bool = False) -> AsyncIterator[tuple[Any, str]]:
        async for item in self._connect_raw(f"{WS_MARKET_RAW_BASE_URL}/{stream}", stream, emit_reconnect):
            yield item
                ...
                async with self._ws_connector(url) as ws:
                    ...
                    async for raw_message in ws:
                        ...
                        if direct_stream is not None:
                            ...
                            yield envelope.get("data", envelope), envelope.get("stream", "")
