...
class BinanceFuturesAdapter(ExchangeAdapter):
    ...
    async def stream_trades(self, symbols: list[str]) -> AsyncIterator[TradeTick]:
        ...
        fallback_streams = {symbol: self._direct_raw_stream(f"{symbol.lower()}@aggTrade", emit_reconnect=True).__aiter__() for symbol in normalized_symbols}
        loop = asyncio.get_running_loop()
        primary_last_data: dict[str, float] = {symbol: loop.time() for symbol in normalized_symbols}
        fallback_active: set[str] = set()
        fallback_tasks: dict[str, asyncio.Task | None] = {symbol: None for symbol in normalized_symbols}
        ...
        def activate_fallback(symbol: str) -> None:
            ...
            if fallback_tasks[symbol] is None:
                fallback_tasks[symbol] = asyncio.create_task(fallback_streams[symbol].__anext__())
            logger.warning("Binance aggregate-trade stream idle; entering direct per-symbol aggTrade fallback", extra={"aitos_extra": {"symbol": symbol, "idle_seconds": TRADE_STREAM_IDLE_FALLBACK_SECONDS}})
        ...
        try:
            ...
            while True:
                ...
                for symbol in normalized_symbols:
                    if symbol not in fallback_active and now - primary_last_data[symbol] >= TRADE_STREAM_IDLE_FALLBACK_SECONDS:
                        activate_fallback(symbol)
                tasks: set[asyncio.Task] = {primary_task}
                tasks.update(task for task in fallback_tasks.values() if task is not None)
                timeout = TRADE_STREAM_PRIMARY_RETRY_SECONDS
                for symbol in normalized_symbols:
                    if symbol not in fallback_active:
                        remaining = max(0.0, TRADE_STREAM_IDLE_FALLBACK_SECONDS - (loop.time() - primary_last_data[symbol]))
                    ...
                    try:
                        ...
                        if symbol in symbol_set:
                            try:
                                ...
                                logger.error("Binance aggregate-trade event invalid; keeping primary under watchdog", extra={"aitos_extra": {"symbol": symbol, "error": str(exc)}})
                            else:
                                ...
                                if symbol in fallback_active:
                                    ...
                                    logger.info("Binance aggregate-trade stream recovered; returning from direct per-symbol fallback", extra={"aitos_extra": {"symbol": symbol}})
                                yield tick
                                continue
                ...
                for symbol in normalized_symbols:
                    ...
                    try:
                        ...
                        fallback_tasks[symbol] = None
                        logger.error("Binance direct aggTrade fallback stream failed", extra={"aitos_extra": {"symbol": symbol, "error": str(exc)}})
                    finally:
            ...
            if primary_task is not None and not primary_task.done():
                primary_task.cancel()
            await asyncio.gather(*(task for task in [primary_task] if task is not None), return_exceptions=True)
            for task in fallback_tasks.values():
                if task is not None and not task.done():
                    task.cancel()
            await asyncio.gather(*(task for task in fallback_tasks.values() if task is not None), return_exceptions=True)
    ...
    async def _direct_raw_stream(self, stream: str, emit_reconnect: bool = False) -> AsyncIterator[tuple[Any, str]]:
        ...
        while True:
            try:
                ...
                if emit_reconnect:
                    logger.warning("Binance direct market stream closed, reconnecting", extra={"aitos_extra": {"stream": stream, "backoff_seconds": backoff}})
            except asyncio.CancelledError:
                raise
            except Exception as exc:
                logger.error("Binance direct market stream disconnected, reconnecting", extra={"aitos_extra": {"stream": stream, "error": str(exc), "backoff_seconds": backoff}})
