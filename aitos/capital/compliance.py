...
class PropComplianceEngine:
    ...
    def evaluate(
        ...
        if not rules.automation_allowed:
            return ComplianceDecision(False, "prop profile does not allow automated trading")
        if rules.daily_loss_limit is not None and account.daily_loss + max(0.0, projected_loss) >= rules.daily_loss_limit:
            return ComplianceDecision(False, "daily loss limit would be breached")
        if rules.max_drawdown is not None and account.drawdown + max(0.0, projected_loss) >= rules.max_drawdown:
        ...
        if rules.max_order_notional is not None and notional > rules.max_order_notional:
            return ComplianceDecision(False, "order notional exceeds prop profile limit")
