...
class FundingBasisStrategy(Strategy):
    ...
    def evaluate(self, context: StrategyContext) -> StrategyResult:
        for snap in context.snapshots.values():
            ...
            if snap.funding_rate < self.min_funding_rate or edge_bps < self.min_edge_bps:
                continue
            notional = min(context.available_capital * 0.20, context.risk_budget)
            if notional <= 0:
                continue
            return StrategyResult(
                self.strategy_id, self.family,
                ...
                CapitalRequest(self.strategy_id, notional, notional * 0.01, edge_bps / 10_000),
        ...
        for snap in context.snapshots.values():
            ...
            return StrategyResult(
                ...
                (ExecutionIntent(snap.instrument_id, side, notional / snap.mid,
                    strategy_id=self.strategy_id, rationale=f"spread z-score={z:.2f}"),),
...
class MarketMakingStrategy(Strategy):
    ...
    def evaluate(self, context: StrategyContext) -> StrategyResult:
        for snap in context.snapshots.values():
            ...
            return StrategyResult(
                self.strategy_id, self.family,
                (ExecutionIntent(snap.instrument_id, "buy", qty, "limit", snap.mid - half,
                    strategy_id=self.strategy_id, rationale="liquid spread capture"),
                 ExecutionIntent(snap.instrument_id, "sell", qty, "limit", snap.mid + half,
                    strategy_id=self.strategy_id, rationale="liquid spread capture")),
                CapitalRequest(self.strategy_id, quote, quote * 0.01, snap.spread_bps / 10_000),
...
class HedgeStrategy(Strategy):
    ...
    def evaluate(self, context: StrategyContext) -> StrategyResult:
        ...
        if snap is None or snap.mid <= 0:
            return StrategyResult(self.strategy_id, self.family, diagnostics={"reason": "hedge_market_unavailable"})
        side = "sell" if context.portfolio_delta > 0 else "buy"
        return StrategyResult(
            self.strategy_id, self.family,
            (ExecutionIntent(self.hedge_instrument, side, abs(context.portfolio_delta),
                reduce_only=False, strategy_id=self.strategy_id, hedge_group="portfolio-delta",
...
class OptionsVolatilityStrategy(Strategy):
    ...
    def evaluate(self, context: StrategyContext) -> StrategyResult:
        for snap in context.snapshots.values():
            ...
            return StrategyResult(
                self.strategy_id, self.family,
                (ExecutionIntent(snap.instrument_id, "sell", notional / snap.mid,
                    strategy_id=self.strategy_id, hedge_group=f"option-vol:{snap.instrument_id}",
...
class RegimeRouterStrategy(Strategy):
    ...
    ROUTES = {
        ...
        "sideways": (StrategyFamily.MARKET_MAKING, StrategyFamily.FUNDING_BASIS),
