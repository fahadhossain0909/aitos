...
class DurableTradingStateStore:
    ...
    async def save_trade(self, trade: Trade) -> None:
        ...
        await client.insert(
            ...
            column_names=["trade_id", "symbol", "state", "payload_json", "updated_at"],
        )

    async def delete_trade(self, trade_id: str) -> None:
        client = self._client()
        await client.command(
            "ALTER TABLE trade_runtime_state DELETE WHERE trade_id = {trade_id:String}",
    ...
    async def load_open_trades(self) -> List[Trade]:
        ...
        for row in result.result_rows:
            try:
                ...
                logger.error("failed to restore trade state", extra={"aitos_extra": {"error": str(exc)}})
        return trades
    ...
    async def save_drawdown(self, asset: str, equity: float, peak: float) -> None:
        ...
        await client.insert(
            ...
            column_names=["asset", "time", "equity_usd", "peak_equity_usd", "drawdown_pct"],
        )
    ...
    def _client(self) -> Any:
        if self._repository is None or self._repository._client is None:
            raise RuntimeError("durable trading state requires an initialized ClickHouse repository")
...
class TradeStatePersistence:
    ...
    def __init__(self, event_bus: EventBus, lifecycle: Any, store: DurableTradingStateStore) -> None:
    ...
    async def restore(self) -> int:
        ...
        if trades:
            logger.warning("restored open trades after process restart", extra={"aitos_extra": {"count": len(trades)}})
        return len(trades)

    async def initialize(self) -> None:
        await self._store.initialize()
        self._subscriptions = [
            await self._event_bus.subscribe("trade.position_opened", self._save_event, group="trade-state-persistence"),
            await self._event_bus.subscribe("trade.position_updated", self._save_event, group="trade-state-persistence"),
            await self._event_bus.subscribe("trade.trailing_sl", self._save_event, group="trade-state-persistence"),
            await self._event_bus.subscribe("trade.partial_close", self._save_event, group="trade-state-persistence"),
            await self._event_bus.subscribe("trade.position_closed", self._delete_event, group="trade-state-persistence"),
        ]
    ...
    async def _save_event(self, event: Any) -> None:
        try:
            ...
            if trade.state in (TradeLifecycleState.POSITION_OPENED, TradeLifecycleState.EXIT_TRIGGERED):
                await self._store.save_trade(trade)
        except Exception as exc:
            logger.error("trade state persistence failed", extra={"aitos_extra": {"error": str(exc)}})
...
class PersistentLivePortfolioTracker(LivePortfolioTracker):
    ...
    def __init__(self, order_executor: Any, state_store: DurableTradingStateStore, asset: str = "USDT") -> None:
...
class IdempotentOrderExecutor(OrderExecutor):
    ...
    async def submit_order(self, request: OrderRequest) -> OrderResult:
        ...
        request = request if request.client_order_id else replace(request, client_order_id=key)
    ...
    def _key(self, request: OrderRequest) -> str:
        raw = "|".join(
            [request.symbol, request.side.value, f"{request.quantity:.12g}", f"{request.reference_price:.12g}", request.order_type]
        )
    ...
    async def get_resting_order_status(self, *args: Any, **kwargs: Any) -> Optional[str]:
