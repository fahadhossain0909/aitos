...
class PositionManager:
    ...
    def _journey_score_inputs(market_state: MarketState) -> tuple[float, float, float]:
        momentum = {"STRONG": 1.0, "MODERATING": 0.68, "WEAK": 0.38, "EXHAUSTED": 0.15}.get(market_state.momentum.value, 0.5)
        liquidity = {"UPSIDE_LIQUIDITY_HIGH": 0.85, "DOWNSIDE_LIQUIDITY_HIGH": 0.85, "BALANCED": 0.65, "THIN": 0.25, "UNKNOWN": 0.50}.get(market_state.liquidity_bias.value, 0.5)
        structure = {"BULLISH": 0.90, "BEARISH": 0.90, "RANGE": 0.60, "BROKEN": 0.10}.get(market_state.structure.value, 0.5)
    ...
    def evaluate(self, *, trade: Trade, current_price: float, order_flow: OrderFlowFeatures | None = None,
        ...
        momentum_score, liquidity_score, structure_score = self._journey_score_inputs(market_state)
        journey = self._journey_engine.evaluate(
            ...
            thesis_health=thesis_health, momentum=momentum_score, liquidity=liquidity_score,
            structure=structure_score, expected_path_prices=expected_path,
        )
