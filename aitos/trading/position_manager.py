...
class PositionManager:
    def __init__(self, market_state_engine=None, path_planner=None, structural_risk_engine=None, exit_intelligence_engine=None, thesis_engine=None, hedge_intelligence_engine=None, config: Mapping[str, Any] | None = None) -> None:
    ...
    def evaluate(self, *, trade: Trade, current_price: float, order_flow: OrderFlowFeatures | None = None, volume_profile: VolumeProfile | None = None, liquidity_events: Sequence[LiquidityEvent] = (), prior_highs: Sequence[float] = (), prior_lows: Sequence[float] = (), swing_highs: Sequence[float] = (), swing_lows: Sequence[float] = (), structure_break_level: float | None = None, atr: float | None = None, trend_strength: float | None = None, extra_features: Mapping[str, float] | None = None, timestamp: datetime | None = None) -> PositionAction:
        ts = timestamp or datetime.now(timezone.utc)
        side = trade.side.value
        market_state = self._mse.compute(symbol=trade.symbol, mid_price=current_price, order_flow=order_flow, trend_strength=trend_strength, atr_pct=(atr / current_price * 100.0) if atr and current_price > 0 else None, volume_profile_poc=volume_profile.poc if volume_profile else None, value_area_high=volume_profile.vah if volume_profile else None, value_area_low=volume_profile.val if volume_profile else None, structure_bias_hint=None, timestamp=ts, extra_features=extra_features)
        path_plan = self._mpp.plan(market_state=market_state, volume_profile=volume_profile, liquidity_events=liquidity_events, prior_highs=prior_highs, prior_lows=prior_lows, swing_highs=swing_highs, swing_lows=swing_lows)
        structural_stop = self._sre.compute(symbol=trade.symbol, side=side, entry_price=trade.entry_price, market_state=market_state, volume_profile=volume_profile, swing_lows=swing_lows, swing_highs=swing_highs, structure_break_level=structure_break_level, liquidity_events=liquidity_events, atr=atr, timestamp=ts)
        ...
        if thesis is None:
            ...
            thesis = self._thesis_engine.build_from_entry(trade_id=trade.trade_id, symbol=trade.symbol, side=side, entry_price=trade.entry_price, market_state=market_state, structural_invalidation_price=structural_stop.stop_price, expected_path_prices=expected, strategy_id=trade.strategy_id, rationale=trade.explanation or "", timestamp=ts)
        ...
        exit_decision = self._eie.evaluate(symbol=trade.symbol, side=side, entry_price=trade.entry_price, current_price=current_price, market_state=market_state, path_plan=path_plan, structural_stop=structural_stop, thesis=thesis, thesis_eval=thesis_eval, timestamp=ts)
        hedge_decision = self._hedge_engine.evaluate(trade=trade, market_state=market_state, current_price=current_price, timestamp=ts, atr=atr) if self._hedge_enabled and exit_decision.action != ExitAction.EXIT else None
        new_stop = None
        ...
        return PositionAction(action=exit_decision.action, reason=reason, reduce_fraction=exit_decision.suggested_reduce_fraction, new_stop_price=new_stop, exit_decision=exit_decision, hedge_decision=hedge_decision, path_plan=path_plan, structural_stop=structural_stop, market_state=market_state, thesis=thesis, thesis_eval=thesis_eval, notes=exit_decision.notes)
