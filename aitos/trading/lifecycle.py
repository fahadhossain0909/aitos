...
class TradeLifecycle(AITOSModule):
    def __init__(self, event_bus: EventBus, risk_engine: RiskEngine, order_executor: OrderExecutor | None = None, kernel: AIKernel | None = None, use_exchange_side_stops: bool = False) -> None:
    ...
    async def health_check(self) -> HealthStatus:
        return HealthStatus(module_id=self.module_id, status=ModuleStatus.HEALTHY if self._initialized else ModuleStatus.UNHEALTHY, latency_ms=0.0, last_event_time=self._last_event_time, details={"open_trades": len(self._open_trades), "closed_trades": len(self._closed_trades)})
    ...
    async def handle_event(self, event: Event) -> EventResponse | None:
        ...
        if event.topic.startswith("market.kline.") or event.topic.startswith("market.trade."):
            ...
            if symbol and _valid_market_price(price):
                ...
                logger.warning("Ignoring invalid market price", extra={"aitos_extra": {"symbol": symbol, "price": price}})
        return None
    ...
    async def submit_opportunity(self, opportunity: Opportunity, portfolio: PortfolioState) -> Trade:
        ...
        await self._event_bus.publish(Event(topic=TOPIC_OPPORTUNITY, payload={"symbol": opportunity.symbol, "side": opportunity.side.value, "confidence": opportunity.confidence}, source_module=self.module_id))
        ...
        if opportunity.is_production and self._kernel is not None:
            action = Action(action_type="order.submit", payload={"symbol": opportunity.symbol, "side": opportunity.side.value}, is_production=True, approved_by=opportunity.approved_by)
    ...
    async def update_price(self, trade_id: str, current_price: float) -> Trade:
        ...
        if not _valid_market_price(current_price):
            raise ValueError(f"current_price must be a finite positive number, got {current_price!r}")
        ...
        if trade.trailing_sl_enabled:
            ...
            if improved:
                ...
                if self._use_exchange_side_stops:
                    await self._replace_exchange_side_stop_loss(trade)
                await self._event_bus.publish(Event(topic=TOPIC_TRAILING_SL_UPDATE, payload={**trade.to_dict()}, source_module=self.module_id))
        return trade
    ...
    async def close_trade(self, trade_id: str, exit_price: float, reason: str) -> Trade:
        ...
        initial_position_size_usd = trade.position_size_usd + sum(pe.size_usd for pe in trade.partial_exits)
        trade.pnl_percent = round((trade.pnl / initial_position_size_usd) * 100, 4) if initial_position_size_usd > 0 else 0.0
    ...
    async def _place_exchange_side_stops(self, trade: Trade) -> None:
        ...
        await self._event_bus.publish(Event(topic=TOPIC_EXCHANGE_STOPS_PLACED, payload={**trade.to_dict()}, source_module=self.module_id))
    ...
    def _project_portfolio(self, portfolio: PortfolioState, opportunity: Opportunity, sizing: PositionSizeResult) -> PortfolioState:
        candidate = PositionExposure(symbol=opportunity.symbol, notional_usd=sizing.position_size_usd, leverage=sizing.leverage)
        return PortfolioState(equity_usd=portfolio.equity_usd, peak_equity_usd=portfolio.peak_equity_usd, positions=(*portfolio.positions, candidate), daily_pnl_pct=portfolio.daily_pnl_pct, weekly_pnl_pct=portfolio.weekly_pnl_pct, volatility_percentile=portfolio.volatility_percentile, regime=portfolio.regime, max_pairwise_correlation=portfolio.max_pairwise_correlation, api_error_rate_pct=portfolio.api_error_rate_pct, api_latency_ms=portfolio.api_latency_ms, data_freshness_seconds=portfolio.data_freshness_seconds, model_accuracy=portfolio.model_accuracy, timestamp=portfolio.timestamp)
    ...
    async def _validate_and_open(self, opportunity: Opportunity, portfolio: PortfolioState) -> Trade:
        ...
        sizing = calculate_position_size(equity_usd=portfolio.equity_usd, entry_price=opportunity.entry_price, stop_loss_price=opportunity.stop_loss_price, risk_limits=self._risk_engine.limits, risk_score=risk_score, volatility_percentile=portfolio.volatility_percentile, correlation_penalty=portfolio.max_pairwise_correlation, existing_sector_notional_usd=existing_sector_notional, sector_limit_pct=self._risk_engine.limits.max_sector_exposure_pct)
        ...
        await self._event_bus.publish(Event(topic=TOPIC_ORDER_SUBMITTED, payload={"symbol": opportunity.symbol, "side": opportunity.side.value, "quantity": quantity}, source_module=self.module_id))
        await self._event_bus.publish(Event(topic=TOPIC_ORDER_FILLED, payload={"order_id": order_result.order_id, "fill_price": order_result.fill_price}, source_module=self.module_id))
        trade = Trade(trade_id=new_trade_id(), symbol=opportunity.symbol, side=opportunity.side, entry_price=order_result.fill_price, quantity=order_result.filled_quantity, leverage=sizing.leverage, position_size_usd=sizing.position_size_usd, risk_amount_usd=sizing.risk_amount_usd, strategy_id=opportunity.strategy_id, agent_consensus=opportunity.agent_consensus, explanation=opportunity.rationale, sl_price=opportunity.stop_loss_price, tp_price=opportunity.take_profit_levels[0] if opportunity.take_profit_levels else opportunity.entry_price, take_profit_levels=list(opportunity.take_profit_levels), state=TradeLifecycleState.POSITION_OPENED, entry_time=utc_now_iso(), trailing_sl_enabled=opportunity.trailing_sl_enabled, breakeven_at_r_multiple=opportunity.breakeven_at_r_multiple, regime=opportunity.regime)
    ...
    async def _reject(self, opportunity: Opportunity, reason: str) -> Trade:
        trade = Trade(trade_id=new_trade_id(), symbol=opportunity.symbol, side=opportunity.side, entry_price=opportunity.entry_price, quantity=0.0, leverage=0.0, position_size_usd=0.0, risk_amount_usd=0.0, strategy_id=opportunity.strategy_id, agent_consensus=opportunity.agent_consensus, explanation=opportunity.rationale, sl_price=opportunity.stop_loss_price, tp_price=opportunity.take_profit_levels[0] if opportunity.take_profit_levels else opportunity.entry_price, take_profit_levels=list(opportunity.take_profit_levels), state=TradeLifecycleState.REJECTED, entry_time=utc_now_iso(), rejection_reason=reason)
    ...
    async def _partial_close(self, trade: Trade, price: float, close_fraction: float) -> None:
        ...
        if not 0.0 < close_fraction <= 1.0:
            raise ValueError(f"close_fraction must be in (0, 1], got {close_fraction!r}")
        if trade.position_size_usd <= 0.0 or trade.quantity <= 0.0:
            raise ValueError(f"trade has no remaining position to close: {trade.trade_id}")
        direction = 1 if trade.side == TradeSide.LONG else -1
        ...
        trade.partial_exits.append(PartialExit(price=price, size_usd=round(closed_notional, 2), r_multiple=round(trade.unrealized_r_multiple(price), 4)))
        ...
        trade.updated_at = utc_now_iso()
        await self._event_bus.publish(Event(topic=TOPIC_PARTIAL_CLOSE, payload={**trade.to_dict(), "closed_notional_usd": closed_notional, "closed_quantity": closed_quantity, "realized_pnl": realized}, source_module=self.module_id))
