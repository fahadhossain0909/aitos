...
def _local_swing_highs_lows(
    ...
    for i in range(left, n - right):
        ...
        if klines[i].high >= max(window_highs) and klines[i].high == max(
            klines[j].high for j in range(i - left, i + right + 1)
        ):
            highs.append((i, klines[i].high))
        if klines[i].low <= min(window_lows) and klines[i].low == min(
            klines[j].low for j in range(i - left, i + right + 1)
        ):
