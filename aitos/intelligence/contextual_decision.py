...
class ContextualDecisionEngine:
        ...
        *,
        ...
        if direction not in {"long", "short"}:
            return ContextualDecision(
                "no_trade", 0.0, "unknown", (), (), (), ("no directional thesis",), (),
        ...
        scenarios = self._scenarios(direction, directional_confidence, advanced, contradictions)
        best = scenarios[0] if scenarios else None
        action = direction if best and best.probability >= self.min_confidence and not (
    ...
    def _scenarios(
        ...
        continuation = max(0.0, min(1.0, confidence + symmetry_bonus - 0.04 * len(contradictions)))
        reversal = max(0.0, min(1.0, 1.0 - confidence + (0.08 if contradictions else 0.0)))
        range_prob = max(0.0, 1.0 - max(continuation, reversal))
        total = continuation + reversal + range_prob or 1.0
        scenarios = [
            Scenario("continuation", continuation / total, direction, confidence, 1.0 - confidence, ("flow confirmation", "structure remains intact")),
            Scenario("reversal", reversal / total, opposite, 1.0 - confidence, confidence, ("structural break", "opposing flow confirmation")),
            Scenario("range", range_prob / total, "neutral", 0.5, 0.5, ("failed displacement", "balanced auction")),
        ]
