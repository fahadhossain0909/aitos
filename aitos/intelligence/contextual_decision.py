...
class ContextualDecisionEngine:
    DEFAULT_RELIABILITY: Mapping[str, float] = {
        "trend_strength": 0.90, "liquidity_quality": 0.88,
        "order_flow_bias": 0.92, "auction_context": 0.90,
        "volatility": 0.86, "market_regime": 0.88, "lead_lag": 0.80,
        "funding_rate": 0.70, "open_interest_trend": 0.78,
        "rl_confidence": 0.75, "footprint_interaction": 0.93,
        "volume_profile": 0.84, "price_imbalance": 0.62,
        "structural_symmetry": 0.55, "forced_flow": 0.68,
        "historical_analogue": 0.72, "state_transition": 0.78,
        "open_interest_positioning": 0.78, "funding_crowding": 0.70,
        "basis_context": 0.70,
    }
    ...
    def assess_evidence(self, component_scores: Mapping[str, Any], direction: str, *,
                        relevance: Mapping[str, float] | None = None,
                        reliability: Mapping[str, float] | None = None) -> tuple[EvidenceAssessment, ...]:
    ...
    def build(self, *, direction: str, component_scores: Mapping[str, Any],
              component_availability: Mapping[str, bool] | None = None,
              context: Mapping[str, Any] | None = None,
              advanced: AdvancedMarketContext | None = None,
              positioning: PositioningContext | None = None) -> ContextualDecision:
        direction = direction.lower()
        if direction not in {"long", "short"}:
            return ContextualDecision("no_trade", 0.0, "unknown", (), (), (),
                                      ("no directional thesis",), (),
                                      ("No directional thesis was supplied.",))
        context = context or {}
        availability = component_availability or {}
        usable = {k: v for k, v in component_scores.items()
        ...
        if advanced is not None:
            derived = {
                ...
                "volatility": max(0.0, min(10.0, 10.0 - advanced.volatility.percentile / 10.0)),
            }
        ...
        rationale = (
            f"state={current_state}", f"directional_confidence={confidence:.3f}",
            f"evidence_sources={len(evidence)}", f"historical_matches={len(analogues)}",
            f"state_transition={transition.transition_score:.1f}",
            f"contradictions={len(contradictions)}", f"action={action}",
        )
        return ContextualDecision(action, round(final_confidence, 4), current_state,
                                  tuple(scenarios), tuple(evidence), tuple(contradictions),
                                  tuple(invalidations), tuple(targets), rationale,
                                  tuple(analogues), transition.to_dict())

    def _scenarios(self, direction: str, confidence: float,
                   advanced: AdvancedMarketContext | None,
                   analogues: Sequence[HistoricalAnalogue],
                   contradictions: Sequence[str]) -> list[Scenario]:
