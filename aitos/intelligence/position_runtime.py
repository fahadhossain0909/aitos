...
def _deep_priority_order(priorities: dict[str, tuple[PositionMonitorTier, float]] | None = None) -> list[str]:
    source = priorities if priorities is not None else _DEEP_PRIORITY_SYMBOLS
    return [symbol for symbol, (tier, priority) in sorted(source.items(), key=lambda item: (-item[1][1], -int(item[1][0] == PositionMonitorTier.EXIT_CANDIDATE))) if tier in {PositionMonitorTier.WARNING, PositionMonitorTier.EXIT_CANDIDATE}]


def _capital_policy(portfolio: Any) -> dict[str, Any]:
    positions = tuple(getattr(portfolio, "positions", ()) or ())
    return {"open_position_count": len(positions), "monitoring_model": "normal_warning_exit_candidate", "position_sizing_authority": "existing_position_manager_and_risk_engine", "fixed_per_position_capital_pct": None, "fixed_per_position_capital_usd": None}
...
def _install_ingestion_guards() -> None:
    ...
    async def guarded_trade(self: DataIngestionService, symbols: list[str] | tuple[str, ...]) -> bool:
        return await original_trade(self, _merge_symbols(_open_symbols(), list(symbols)))

    async def guarded_kline(self: DataIngestionService, symbols: list[str] | tuple[str, ...]) -> bool:
        return await original_kline(self, _merge_symbols(_open_symbols(), list(symbols)))
...
def _cheap_position_action(tier: PositionMonitorTier, score: float, reasons: tuple[str, ...], market_state: Any = None) -> PositionAction:
    return PositionAction(action=ExitAction.MANAGE, reason=f"POSITION_MONITOR:{tier.value} score={score:.2f} [{', '.join(reasons)}]", market_state=market_state, notes=(f"monitor_tier={tier.value}", *reasons))
...
def _warning_market_state(self: PositionManager, *, trade: Any, current_price: float, kwargs: dict[str, Any]) -> Any:
    try:
        ...
        return self._mse.compute(symbol=trade.symbol, mid_price=current_price, order_flow=kwargs.get("order_flow"), trend_strength=kwargs.get("trend_strength"), atr_pct=(atr / current_price * 100.0) if atr and current_price > 0 else None, volume_profile_poc=volume_profile.poc if volume_profile else None, value_area_high=volume_profile.vah if volume_profile else None, value_area_low=volume_profile.val if volume_profile else None, structure_bias_hint=None, timestamp=kwargs.get("timestamp") or datetime.now(timezone.utc), extra_features=kwargs.get("extra_features"))
    except Exception:
        return None
...
def _install_position_monitor() -> None:
    ...
    def guarded_evaluate(self: PositionManager, *, trade: Any, current_price: float, extra_features: Any = None, **kwargs: Any) -> PositionAction:
        if not getattr(self, "_aitos_tiered_monitoring_enabled", False):
            return original_evaluate(self, trade=trade, current_price=current_price, extra_features=extra_features, **kwargs)
        ...
        if decision.tier == PositionMonitorTier.WARNING:
            return _cheap_position_action(decision.tier, decision.score, decision.reasons, _warning_market_state(self, trade=trade, current_price=current_price, kwargs={**kwargs, "extra_features": extra_features}))
        action = original_evaluate(self, trade=trade, current_price=current_price, extra_features=extra_features, **kwargs)
        return PositionAction(action=action.action, reason=f"POSITION_MONITOR:{decision.tier.value} priority={decision.priority_score:.2f} score={decision.score:.2f}; {action.reason}", reduce_fraction=action.reduce_fraction, new_stop_price=action.new_stop_price, spike_tp_price=action.spike_tp_price, exit_decision=action.exit_decision, hedge_decision=action.hedge_decision, path_plan=action.path_plan, structural_stop=action.structural_stop, market_state=action.market_state, thesis=action.thesis, thesis_eval=action.thesis_eval, journey=action.journey, notes=(f"monitor_tier={decision.tier.value}", f"priority_score={decision.priority_score:.2f}", *decision.reasons, *action.notes))
...
def _install_lifecycle_telemetry() -> None:
    ...
    if callable(original_update_price):
        ...
        async def traced_update_price(self: TradeLifecycle, *args: Any, **kwargs: Any) -> Any:
            before_trades = list(self.get_open_trades())
            before_states = {str(getattr(t, "trade_id", "")): str(getattr(getattr(t, "state", None), "value", getattr(t, "state", None))) for t in before_trades}
            started = datetime.now(timezone.utc)
            try:
                result = await original_update_price(self, *args, **kwargs)
                after_trades = list(self.get_open_trades())
                logger.info("lifecycle root-cause telemetry", extra={"aitos_extra": {"stage": "lifecycle_update_price", "open_before": len(before_trades), "open_after": len(after_trades), "open_delta": len(after_trades) - len(before_trades), "state_before_counts": _state_counts(before_states.values()), "state_after_counts": _state_counts(str(getattr(getattr(t, "state", None), "value", getattr(t, "state", None))) for t in after_trades), "duration_ms": (datetime.now(timezone.utc) - started).total_seconds() * 1000.0}})
                return result
            except Exception as exc:
                logger.error("lifecycle update_price failed", extra={"aitos_extra": {"stage": "lifecycle_update_price_error", "open_before": len(before_trades), "error_type": type(exc).__name__, "error": str(exc), "duration_ms": (datetime.now(timezone.utc) - started).total_seconds() * 1000.0}})
                raise
    ...
    if original_submit is not None:
        ...
        async def guarded_submit(self: TradeLifecycle, *args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                logger.error("lifecycle submit telemetry: submit failed", extra={"aitos_extra": {"stage": "lifecycle_submit_error", "open_before": before, "open_after": len(self.get_open_trades()), "error_type": type(exc).__name__, "error": str(exc)}})
                raise
            state = getattr(getattr(trade, "state", None), "value", "")
            symbol = str(getattr(trade, "symbol", "") or "").upper()
            logger.info("lifecycle submit telemetry", extra={"aitos_extra": {"stage": "lifecycle_submit", "symbol": symbol, "trade_id": getattr(trade, "trade_id", None), "result_state": state, "open_before": before, "open_after": len(self.get_open_trades())}})
            if state == "position_opened" and symbol:
                for ingestion in list(_INGESTIONS):
                    try:
                        ...
                        logger.info("position market-data subscription telemetry", extra={"aitos_extra": {"stage": "position_market_data_subscription", "symbol": symbol, "trade_id": getattr(trade, "trade_id", None), "trade_stream_updated": bool(trade_ok), "kline_stream_updated": bool(kline_ok)}})
                    except Exception as exc:
                        logger.exception("position market-data subscription failed", extra={"aitos_extra": {"stage": "position_market_data_subscription_error", "symbol": symbol, "trade_id": getattr(trade, "trade_id", None), "error_type": type(exc).__name__, "error": str(exc)}})
            return trade
