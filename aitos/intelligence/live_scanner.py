...
class LiveSymbolCache:
    ...
    # Scanner freshness is based on when AITOS received the event.
...
class LiveScannerCache:
    ...
    async def _on_trade(self, event: Event) -> None:
        ...
        if source_age > LIVE_TRADE_MAX_AGE_SECONDS:
            logger.info("ignored stale trade in live scanner", extra={"aitos_extra": {"symbol": trade.symbol, "trade_id": trade.trade_id, "trade_age_sec": round(max(0.0, source_age), 3), "max_age_seconds": LIVE_TRADE_MAX_AGE_SECONDS}})
            return
        state = self._cache(trade.symbol)
        state.trades.append(trade)
        # IMPORTANT: scanner stale checks use receive time, not exchange time.
    ...
    def freshness_snapshot(self, symbol: str) -> dict:
        """Expose source age and receive age for diagnostics."""
        state = self._state.get(symbol)
        if state is None:
            return {"cache_has_state": False, "last_trade_at": None, "last_book_at": None, "last_trade_received_at": None, "last_book_received_at": None, "trade_age_sec": None, "book_age_sec": None, "trade_receive_age_sec": None, "book_receive_age_sec": None, "trade_consumer_lag_sec": None, "book_consumer_lag_sec": None}
        ...
        return {
            ...
            "trade_consumer_lag_sec": 0.0 if trade_age is not None else None,
            "book_consumer_lag_sec": 0.0 if book_age is not None else None,
        }
