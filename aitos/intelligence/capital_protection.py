...
This module is deliberately venue- and strategy-neutral. It adds four hard
controls around the capital objective: concentration/correlation, dynamic
regime/volatility sizing, drawdown protection, and capital reservation.
"""
...
class ProtectionConfig:
    """Conservative defaults; calibrate with paper-trading/backtest data."""
    ...
    def __post_init__(self) -> None:
        for name in (
            "max_portfolio_risk_pct", "max_correlated_risk_pct",
            "max_single_cluster_pct", "drawdown_1_pct", "drawdown_2_pct",
            "drawdown_3_pct", "drawdown_stop_pct", "drawdown_mult_1",
            "drawdown_mult_2", "drawdown_mult_3", "volatility_mult_high",
...
class PortfolioRiskSnapshot:
    """Current portfolio risk plus optional pairwise correlations.

    ``position_risk_pct`` is risk budget, not gross notional. Unknown
    correlations are intentionally conservative rather than treated as zero.
    """
...
class PortfolioProtection:
    """Final portfolio-level veto/reduction policy."""
    ...
    def evaluate(
        ...
        *,
        ...
        allowed_risk = min(
            requested_risk_pct * multiplier,
            portfolio_remaining,
            max(0.0, self.config.max_correlated_risk_pct - existing_correlated),
        )
        if allowed_risk <= 0:
...
class CapitalReservation:
    """Thread-safe reservation ledger preventing simultaneous oversubscription."""
