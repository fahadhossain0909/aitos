...
class MarketDataRepository(AITOSModule):
    ...
    async def save_market_event(self, event: Any) -> None:
        self._require_initialized()
        await self._client.insert("market_events", [[event.event_time, event.ingest_time, event.event_id, event.exchange, event.market, event.symbol, event.event_type.value, event.source.value, event.sequence, event.schema_version, event.correlation_id or event.event_id, event.trace_id or event.event_id, json.dumps(event.payload, sort_keys=True, default=str)]], column_names=["event_time", "ingest_time", "event_id", "exchange", "market", "symbol", "event_type", "source", "sequence", "schema_version", "correlation_id", "trace_id", "payload_json"])
    ...
    async def save_live_analytics_event(self, *, category: str, symbol: str, payload: dict[str, Any], event_time: Any, ingest_time: Any | None = None, event_id: str | None = None, exchange: str = "unknown", market: str = "unknown", source_module: str = "unknown", correlation_id: str | None = None, schema_version: str = "1.0") -> None:
        ...
        await self._client.insert("live_analytics_events", [[event_time, ingest_time, event_id, category, exchange, market, symbol, source_module, correlation_id or event_id, schema_version, json.dumps(payload, sort_keys=True, default=str)]], column_names=["event_time", "ingest_time", "event_id", "category", "exchange", "market", "symbol", "source_module", "correlation_id", "schema_version", "payload_json"])
    ...
    async def save_order_book_snapshot(self, book: OrderBookSnapshot) -> None:
        await self.save_order_book_snapshots([book])

    async def save_orderbook_snapshot(self, book: OrderBookSnapshot) -> None:
        await self.save_order_book_snapshot(book)

    async def save_order_book_snapshots(self, books: list[OrderBookSnapshot]) -> None:
        self._require_initialized()
        if not books:
            return
        rows = [[book.timestamp, book.symbol, json.dumps(book.bids), json.dumps(book.asks), book.spread, book.depth_ratio, book.last_update_id] for book in books]
        await self._client.insert("order_book_snapshots", rows, column_names=["time", "symbol", "bid_levels", "ask_levels", "spread", "depth_ratio", "last_update_id"])

    async def save_trade_tick(self, trade: TradeTick) -> None:
        await self.save_trade_ticks([trade])

    async def save_trade_ticks(self, trades: list[TradeTick]) -> None:
        self._require_initialized()
        if not trades:
            return
        rows = [[trade.timestamp, trade.symbol, trade.trade_id, trade.price, trade.quantity, trade.side.value, int(trade.is_buyer_maker)] for trade in trades]
        await self._client.insert("trade_ticks", rows, column_names=["time", "symbol", "trade_id", "price", "quantity", "side", "is_buyer_maker"])
