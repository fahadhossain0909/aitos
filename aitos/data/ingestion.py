...
class DataIngestionService(AITOSModule):
    ...
    async def health_check(self) -> HealthStatus:
        ...
        # Intentional diagnostic: a finished (non-cancelled) stream task means
        # market data is not fully flowing — report UNHEALTHY so audit/CD sees it.
        # Historical error counters stay in details; they must not alone keep the
        # module UNHEALTHY after the task has been restarted and is alive again.
        ...
        while True:
            try:
                async for kline in self._exchange.stream_klines(
                    ...
                    await self._handle_kline(kline)
                # Generator ended cleanly (exchange closed). Restart.
                self._errors += 1
                logger.warning(
                    "kline stream ended without error; restarting",
                ...
                self._errors += 1
                logger.exception(
                    "kline stream loop crashed; restarting",
                    extra={
                        "aitos_extra": {
    ...
    async def _run_orderbook_stream(self) -> None:
        ...
        while True:
            try:
                ...
                # Generator ended cleanly (WS closed / bootstrap ended). Restart.
                self._errors += 1
                logger.warning(
                    "order book stream ended without error; restarting",
                    extra={
                        "aitos_extra": {
                            "symbols": self._symbols,
                            "levels": self._orderbook_levels,
                        }
                    },
                )
                await asyncio.sleep(STREAM_RESTART_DELAY_SECONDS)
                ...
                self._errors += 1
                logger.exception(
                    "order book stream loop crashed; restarting",
                    extra={
                        "aitos_extra": {
                            "error_type": type(exc).__name__,
                            "error": str(exc),
